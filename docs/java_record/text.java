...
/**
 * ============================================================
 *  Mini Record Compiler
 * ============================================================
 *  실제 javac가 record 키워드를 만나면 하는 일을
 *  순수 Java로 재현한 미니 컴파일러입니다.
 *
...
public class MiniRecordCompiler {
    ...
    static class RecordDeclaration {
        ...
        String compactConstructorBody = null; // 컴팩트 생성자 본문

        RecordDeclaration(String name) {
            this.name = name;
        }
    }
    ...
    // ─────────────────────────────────────────────
    // 5단계: 코드 생성기 (Code Generator)
    //   ★ 이 부분이 javac가 실제로 하는 일의 핵심 ★
    // ─────────────────────────────────────────────
    static class CodeGenerator {
        ...
        CodeGenerator(RecordDeclaration decl) {
            this.decl = decl;
        }
        ...
        // ── 클래스 선언 ──
        //  record는 항상 final이고 java.lang.Record를 상속한다
        private void generateClassHeader() {
            StringBuilder header = new StringBuilder();
            header.append("public final class ").append(decl.name);
        ...
        private void generateConstructor() {
            ...
            // ★ 컴팩트 생성자 본문 삽입 (사용자가 작성한 검증/정규화 로직)
            if (decl.compactConstructorBody != null) {
                line("// ── 사용자 작성 컴팩트 생성자 본문 (필드 할당 전에 실행) ──");
        ...
        // ── 접근자 메서드 ──
        //  getX()가 아니라 x() 형태임에 주의
        private void generateAccessors() {
            line("// ── 컴파일러 생성: 접근자 메서드 (getter가 아닌 컴포넌트명 그대로) ──");
            for (RecordComponent comp : decl.components) {
        ...
        // ── toString ──
        //  형식: ClassName[field1=value1, field2=value2]
        private void generateToString() {
            line("// ── 컴파일러 생성: toString ──");
            line("@Override");
